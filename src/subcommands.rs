...
use crate::utils::{request, to_html, lang_select, set_item_toml, make_client, CREATE_ERR, OPEN_ERR, READ_ERR, WRITE_ERR};
...
pub fn init() {
    ...
    File::create("./.attest/link.txt").expect(CREATE_ERR);
...
pub async fn login(user_name: String, password: String) {
    ...
    let csrf_token = html.select(&selector).next().unwrap().attr("value").unwrap();
    ...
    if response.url().path() == "/" {
        ...
        let login_cookie = cookies.iter().find(|&v| v.name() == "REVEL_SESSION").unwrap();
        ...
        let mut file = File::create(format!("{}/.attest_global/cookies.txt",home_dir_txt)).expect(CREATE_ERR);
...
pub async fn lang(lang: Option<String>, list: bool, url: Option<String>, search: Option<String>) -> Result<()> {
    ...
    let langs = match url {
        ...
        None => {
            let past_contests = request(&client,"https://atcoder.jp/contests/archive?ratedType=1&category=0&keyword=").await?;

            let html = to_html(past_contests);

            let selector = Selector::parse(r#"div[class="table-responsive"] tbody td span + a"#).unwrap();

            let url_elem = html.select(&selector).next().unwrap();

            let url = String::from("https://atcoder.jp") + url_elem.attr("href").unwrap() + "/submit";
    ...
    if list {
        ...
        for (lang,_) in &langs {
            if lang.to_ascii_lowercase().contains(&lang_name.to_ascii_lowercase()) {
                println!("{}",lang);
            }
        }
